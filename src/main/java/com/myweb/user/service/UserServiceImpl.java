...
public class UserServiceImpl implements UserService{
	...
	public void join(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (cnt == 1) { //아이디 중복
			...
			response.sendRedirect("login.user"); //MVC2 방식에서 리다이렉트는 다시 컨트롤러를 태울 때 사용합니다.
		}
	}
	...
	public void getInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//아이디 기반으로 회원정보를 조회해서 데이터를 가지고, 수정 페이지로 이동
		//실습...
		/*
		 * 1. 아이디는 세션이 있습니다.
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 		=> login메서드는 id, pw를 받아서 비밀번호가 일치하는지 확인할 수 있습니다. (재활용)
		 * 
		 * 3. login메서드가 DTO 반환하면 DAO에 delete()메서드를 만들고 회원 삭제를 진행하면 됩니다.
		 * 4. 삭제 성공시에는 세션을 전부 삭제하고 "홈화면" 으로 redirect,
		 *    login메서드가 null을 반환하면 delete.jsp에 "비밀번호를 확인하세요" 메세지를 남겨주면 됩니다. 
		 */
		HttpSession session = request.getSession();
		String id = (String)session.getAttribute("user_id");
		String pw = request.getParameter("pw");
		...
		if(dto != null) {//pw가 일치한다는 의미
			...
			request.setAttribute("msg", "아이디 또는 비밀번호를 확인하세요");
			request.getRequestDispatcher("delete.jsp").forward(request, response); //msg데이터를 보낼거기에 forward
